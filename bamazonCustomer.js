...
function readProducts() {
    ...
    connection.query("SELECT item_id, product_name, department_name, price, quantity FROM products", function (err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            var tableArray = [res[i].item_id, res[i].product_name, res[i].price, res[i].quantity];
...
function userPurchase() {
    inquirer.prompt([
        ...
        if (parseInt(inquirerResponse.quantity) <= selectedProduct.quantity) {
            ...
            console.log(msg + "\n");
            connection.query("UPDATE products SET quantity = ? WHERE item_id = ?", [selectedProduct.quantity - inquirerResponse.quantity, inquirerResponse.choiceID], function (err, res) {
                if (err) throw err;
                console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
                console.log("Your total cost is $" + parseInt(selectedProduct.price * inquirerResponse.quantity) + "\n");
                restart();
            })
        }
        else {
            console.log("\n" + "Sorry, " + inquirerResponse.username + "," + " insufficient quantity! Please try again.\n");
            readProducts();
        }
    });
